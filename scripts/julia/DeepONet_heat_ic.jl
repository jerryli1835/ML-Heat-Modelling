...
#function for generating training data using numerical method, using fixed grid sampling
function generate_train_data_fixedgrid(;num=500, grid_size_t=0.5, grid_size_z=0.25, t_domain = (0.0, 100.0), z_domain=(0.0, 10.0))
    ...
    prob, zs = setup_pde_problem(t_domain, z_domain, T0=x -> x, α=0.01)
    ...
    X = zeros((length(grid_z), num))
    for n in 1:num
        input_func(x) = 0.001*n*x
        @info "Loop $n"
        ...
        func_val_n = [input_func(a) for a in grid_z]
        for q in 1:length(grid_z)
            X[q,n] = func_val_n[q]
        end
    end
...
function generate_train_data_quasi(;num=100, num_cord=1024, t_domain = (0.0, 3600), z_domain=(0.0, 10.0))
    ...
    grid_ic = collect(LinRange(z_domain[1], z_domain[2], 41))
    #set up the pde problem (discritise once) for a random condition
    prob, zs = setup_pde_problem(t_domain, z_domain, T0=x -> x, α=0.01)
...
#generate the training data for initial condiditons: ...
X, y, data = generate_train_data_fixedgrid(num=1000)
X, y, data = generate_train_data_quasi(num=1000, num_cord=6000)
u0 = X'
xt = y'
npzwrite("heat_train_linearic_1h_fixed.npz", u0=u0, xt=xt, u=data)


#function for generating testing data using numerical method, using fixed grid sampling
function generate_test_data_fixedgrid(;num=500,  grid_size_t=0.5, grid_size_z=0.25, t_domain = (0.0, 100.0), z_domain=(0.0, 10.0))
    grid_t_num,grid_z_num = Int((t_domain[2]-t_domain[1])/grid_size_t +1), Int((z_domain[2]-z_domain[1])/grid_size_z +1)
    cord = zeros((2, grid_t_num*grid_z_num))
    grid_t = collect(LinRange(t_domain[1], t_domain[end], grid_t_num))
    grid_z = collect(LinRange(z_domain[1], z_domain[end], grid_z_num))
...
function generate_test_data_randomgrid(;num=100, num_cord=2048, t_domain = (0.0, 3600), z_domain=(0.0, 10.0))
    #generate an N by 2 matrix containing the sample points
    cord = QuasiMonteCarlo.sample(num_cord, [t_domain[1], z_domain[1]], [t_domain[2], z_domain[2]], GridSample([60.0,0.1]))
    grid_ic = collect(LinRange(z_domain[1], z_domain[2], 41))
    #set up the pde problem (discritise once) for a random condition
    prob, zs = setup_pde_problem(t_domain, z_domain, T0=x -> x, α=0.01)
...
#generate the testing data for initial condiditons: ...
X_test, y_test, data_test = generate_test_data_fixedgrid(num=150)
X_test, y_test, data_test = generate_test_data_randomgrid(num=200, num_cord=3500)
u0_test = X_test'
xt_test = y_test'
npzwrite("heat_test_linearic_1h_fixed.npz", u0=u0_test, xt=xt_test, u=data_test)
