...
function generate_train_data_fixedgrid(;num=500, grid_size_t=4, grid_size_z=0.5, t_domain = (0.0, 3600.0), z_domain=(0.0, 10.0), kwargs...)
    ...
    for n in 1:num
        ...
        @info "Loop $n"
        new_p = 
        newprob = remake(prob, p=new_p, u0=input_func.(zs))
...
u0 = X'
xt = y'
npzwrite("heat_train_nonlinear_1h_quasi.npz", u0=u0, xt=xt, u=data)


#function for generating testing data using numerical method and fixed grid sampling
function generate_test_data_fixedgrid(;num=500, grid_size_t=4, grid_size_z=0.5, t_domain = (0.0, 3600.0), z_domain=(0.0, 10.0))
    grid_t_num,grid_z_num = Int((t_domain[2]-t_domain[1])/grid_size_t +1), Int((z_domain[2]-z_domain[1])/grid_size_z +1)
    cord = zeros((2, grid_t_num*grid_z_num))
    grid_t = collect(LinRange(t_domain[1], t_domain[end], grid_t_num))
    grid_z = collect(LinRange(z_domain[1], z_domain[end], grid_z_num))
...
X_test, y_test, data_test = generate_test_data_fixedgrid(num=100)
X_test, y_test, data_test = generate_test_data_randomgrid(num=100,  num_cord=8000)
u0_test = X_test'
xt_test = y_test'
npzwrite("heat_test_nonlinear_1h_random.npz", u0=u0_test, xt=xt_test, u=data_test)
